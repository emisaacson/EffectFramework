...
namespace Local.Classes.Builders
{
        ...
        where ModelT : Model, new()
    {
            ...
            where DBT : class, new()
        {
            ...
            if (Models != null)
            {
                foreach (var Item in Models.Items)
                {
                    ...
                    if (((int?)(Item.GetType().GetProperty(PrimaryKey.Name).GetValue(Item))).HasValue)
                    {
                        ...
                        if (EmpGeneralIDPropertyName != null)
                        {
                            DBItem = ((IEnumerable<DBT>)((Table<DBT>)(LocalDB.GetType().GetProperty(this.ContextPropertyName).GetValue(LocalDB)))
                                        .Where(DBPrimaryKey + "=@0 AND " + EmpGeneralIDPropertyName + "=@1", KeyValue, EmployeeLocal.EmpGeneralID)).First();
                        }
                        else
                        {
                            DBItem = ((IEnumerable<DBT>)((Table<DBT>)(LocalDB.GetType().GetProperty(this.ContextPropertyName).GetValue(LocalDB)))
                    ...
                    if (EmpGeneralIDPropertyName != null)
                        DBItem.GetType().GetProperty(EmpGeneralIDPropertyName).SetValue(DBItem, EmployeeLocal.EmpGeneralID);
            ...
            where DBT : class
        {
            ...
            {
                var EmpGeneralID = EmployeeLocal.EmpGeneralID;
                ...
                {
                    var ItemSubset = (IEnumerable<DBT>)this.BuilderSpecification.GetAllItems(this.EmployeeLocal.EmpGeneralID);
            ...
            where DBT : class
        {
            ...
            var EmpGeneralID = this.EmployeeLocal.EmpGeneralID;
            var EmpGeneralIDPropertyName = this.GetEmpGeneralIDPropertyName();
